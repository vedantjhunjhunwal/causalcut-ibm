...
class Settings(BaseSettings):
    ...
    # LLM provider selection: "gemini" (default) or "groq".
    # When set to "groq", CAUSALCUT_GROQ_API_KEY is used and
    # CAUSALCUT_AGENT_MODEL_NAME should be a Groq-hosted model name
    # (e.g. "openai/gpt-oss-120b", "openai/gpt-oss-20b").
    agent_llm_provider: str = Field(default="gemini", description="gemini | groq")
    ...
    # Required when agent_llm_provider="groq".
    groq_api_key: str | None = None

    # --- Supabase -------------------------------------------------------
    # Accepts both CAUSALCUT_SUPABASE_URL (prefixed, auto-generated by pydantic_settings)
    # and the bare SUPABASE_URL that the .env file uses.
    # AliasChoices tries each alias in order; the first match wins.
    supabase_url: str | None = Field(
        default=None,
        validation_alias=AliasChoices("CAUSALCUT_SUPABASE_URL", "SUPABASE_URL"),
    )
    supabase_service_role_key: str | None = Field(
        default=None,
        validation_alias=AliasChoices("CAUSALCUT_SUPABASE_SERVICE_ROLE_KEY", "SUPABASE_SERVICE_ROLE_KEY"),
    )
