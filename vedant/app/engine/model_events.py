...
def generate_model_events(
    ...
    for g in scenario.gas_readings:
        if g.features is None:
            continue  # operator-entered value; lowered by Scenario.to_events()
        try:
            resp = registry.gas.predict(
                g.features, sensor_id=g.sensor_id, zone_id=g.zone_id,
                correlation_id=correlation_id, scenario_id=scenario.scenario_id,
            )
        except InvalidFeaturesError as exc:
            provenance.append({
                "model_name": "gas_xgboost_isoforest", "called": f"gas:{g.sensor_id}",
                "ran": False, "inference_mode": "error", "degraded_reason": str(exc),
                "correlation_id": correlation_id, "scenario_id": scenario.scenario_id,
            })
            continue

        provenance.append(_record(resp, f"gas:{g.sensor_id}", resp.inference_mode == "real"))
        if resp.inference_mode != "real":
            continue  # never fabricate a substitute prediction

        pred = resp.prediction
        events.append(SafetyEvent(
            factory_id=scenario.factory_id,
            zone_id=g.zone_id,
            event_type=pred["event_type"],  # gas_anomaly | sensor_drift, from the model
            event_time=anchor + timedelta(seconds=g.offset_seconds),
            value={
                "sensor_id": g.sensor_id,
                "gas_type": pred["gas_type"],
                "concentration_ppm": pred["concentration_ppm"],
                "drift_detected": pred["drift_detected"],
                "anomaly_score": pred["anomaly_score"],
                "gas_class_probabilities": pred["gas_class_probabilities"],
                # Model attribution: the *classification* is a prediction even
                # though the underlying 128-dim sensor array is measured.
                "model_name": resp.model_name,
                "model_version": resp.model_version,
                "model_prediction": pred["gas_type"],
                "confidence": resp.confidence,
                "inference_mode": resp.inference_mode,
                "latency_ms": resp.latency_ms,
                ...
                # The raw feature vector itself is measured instrument data;
                # only the derived gas class / drift verdict is predicted.
                "raw_input_class": "measured",
            },
            severity=resp.extra.get("severity", 0.0),
            confidence=resp.confidence or 0.0,
            uncertainty=round(1.0 - (resp.confidence or 0.0), 4),
            source=resp.model_name,
            model_version=resp.model_version,
            provenance=resp.artifact_path,
            # XGBoost / IsolationForest output is a MODEL PREDICTION.
