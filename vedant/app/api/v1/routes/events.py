...
from app.services.ingestion import ingest_raw_event
...
async def _ingest_one(
    ...
    """Thin wrapper over the shared ingestion service.

    validate -> clock check -> PERSIST -> queue lives in
    ``app.services.ingestion`` so this route and the scenario pipeline share
    one implementation instead of two that can drift.
    """
    return await ingest_raw_event(raw, correlation_id, events, queue, settings)
