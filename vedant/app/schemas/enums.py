...
# Zone identifier.
#
# The six Steelforge zones below (``ZoneId``) remain the *documented* topology
# and are still used internally for the built-in diffusion/adjacency constants.
# But the platform must also accept fully user-defined scenarios with arbitrary
# zone ids, so the canonical event's ``zone_id`` field is typed as the
# permissive ``ZoneIdStr`` (any short slug) rather than the closed enum. The
# six canonical ids validate under this pattern unchanged, so nothing regresses.
# ---------------------------------------------------------------------------
ZoneIdStr = Annotated[
    str,
    StringConstraints(pattern=r"^[A-Za-z0-9][A-Za-z0-9_.\-]{0,63}$", strip_whitespace=True),
]
